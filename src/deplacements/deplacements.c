...
void deplacer(struct ModeleNiveau *modele, char direction, char *derniere_direction) { // Fonction de deplacement de snoopy
    *derniere_direction = direction; // On enregistre la derniere direction (pour casser les blocs)
    ...
    if((modele->snoopy.x != ancien_snoopy.x || modele->snoopy.y != ancien_snoopy.y) && modele->balle.x == modele->snoopy.x && modele->balle.y == modele->snoopy.y) { // Si la balle a touche snoopy
...
void deplacer_balle(struct ModeleNiveau *modele) { // Fonction de deplacement de la balle
    int add_x = 0; // On initialise les variables d'ajout de coordonnees
    int add_y = 0;
    if(modele->balle.y == modele->hauteur-2 && modele->balle.direction <= 1) { // Si la balle est en bas et qu'elle va vers le bas (traitement Nord/Sud)
        modele->balle.direction = modele->balle.direction == 0 ? 3 : 2; // Sud Est -> Nord Est, Sud Ouest -> Nord Ouest
        return deplacer_balle(modele); // On rappelle la fonction pour deplacer la balle pour retraiter son état et changer sa direciton (est/ouest)
    }
    if(modele->balle.y == 1 && modele->balle.direction > 1) { // Si la balle est en haut et qu'elle va vers le haut (traitement Nord/Sud)
        modele->balle.direction = modele->balle.direction == 2 ? 1 : 0; // Nord Ouest -> Sud Ouest, Nord Est -> Sud Est
        return deplacer_balle(modele); // On rappelle la fonction pour deplacer la balle pour retraiter son état et changer sa direciton (est/ouest)
    }
    if(modele->balle.x == modele->largeur-2 && modele->balle.direction%3 == 0) { // Si la balle est a droite et qu'elle va vers la droite (traitement Est/Ouest)
        modele->balle.direction = modele->balle.direction == 3 ? 2 : 1; // Sud Est -> Sud Ouest, Nord Est -> Nord Ouest
        // Pas besoin de rappeler la fonction car la direction Nord/Sud a deja ete traitee précédemment
    }
    if(modele->balle.x == 1  && modele->balle.direction%3 != 0) { // Si la balle est a gauche et qu'elle va vers la gauche (traitement Est/Ouest)
        modele->balle.direction = modele->balle.direction == 1 ? 0 : 3; // Sud Ouest -> Sud Est, Nord Ouest -> Nord Est
        // Pas besoin de rappeler la fonction car la direction Nord/Sud a deja ete traitee précédemment
    }
    switch (modele->balle.direction) { // On ajoute les coordonnees en fonction de la direction
        case 0: // Sud Est
            add_y = 1; // On ajoute 1 a y pour descendre
            add_x = 1; // On ajoute 1 a x pour aller a droite
            break;
    ...
    modele->balle.y += add_y; // On ajoute les coordonnees
